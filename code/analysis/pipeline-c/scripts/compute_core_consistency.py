...
def main() -> None:
    ...
    output = ROOT / "logs" / "abc_core_consistency_metrics.tsv"
    report.to_csv(output, sep="\t", index=False)
    summary = {
        "scope": "two common-core contrasts only",
        ...
        "metrics_tsv": str(output),
    }
    (ROOT / "logs" / "abc_core_consistency_summary.json").write_text(json.dumps(summary, indent=2))
