#!/usr/bin/env bash
set -euo pipefail
root="<DATA_ROOT>/derivatives/arm-c/mvpa/MVPA_FourDirection_rot_neg90"
...
grep -q "\"n_folds\": 5" "$root/sub-01/metadata.json"
grep -q "\"fold_test_n\": 4" "$root/sub-01/metadata.json"
...
for subject in "${subjects[@]}"; do
    ...
    python "$script" --subject "$subject" --model MVPA_FourDirection_rot_neg90 --n-jobs 12
