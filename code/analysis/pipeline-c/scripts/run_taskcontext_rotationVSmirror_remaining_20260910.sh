#!/usr/bin/env bash
set -euo pipefail
root="<DATA_ROOT>/derivatives/arm-c/mvpa/MVPA_TaskContext_rotationVSmirror"
...
grep -q "\"n_folds\": 16" "$root/sub-01/metadata.json"
...
for subject in "${subjects[@]}"; do
    ...
    python "$script" --subject "$subject" --model MVPA_TaskContext_rotationVSmirror --n-jobs 12
